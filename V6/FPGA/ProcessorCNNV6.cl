...
#define TILE_C 8
#define MAX_WEIGHT_ITERS 576  // 9 * (256 in_c / 4)
#define MAX_ROW_VECS 1280     // 320 width * (16 in_c / 4)
#define MAX_RING_SIZE 3840    // 3 * MAX_ROW_VECS
    ...
    int M_mult, int M_shift)
{
    ...
    // Restauramos a memoria simple: cero multiplexores, área mínima.
    __local short4 ring_buf[MAX_RING_SIZE] __attribute__((memory));
    __local short4 lw0[MAX_WEIGHT_ITERS] __attribute__((memory));
    __local short4 lw1[MAX_WEIGHT_ITERS] __attribute__((memory));
    __local short4 lw2[MAX_WEIGHT_ITERS] __attribute__((memory));
    __local short4 lw3[MAX_WEIGHT_ITERS] __attribute__((memory));
    __local short4 lw4[MAX_WEIGHT_ITERS] __attribute__((memory));
    __local short4 lw5[MAX_WEIGHT_ITERS] __attribute__((memory));
    __local short4 lw6[MAX_WEIGHT_ITERS] __attribute__((memory));
    __local short4 lw7[MAX_WEIGHT_ITERS] __attribute__((memory));
    ...
    for (int t = 0; t < out_c; t += TILE_C) {
        ...
        for (int y = 0; y < out_h; y++) {
            ...
            for (int x = 0; x < out_w; x++) {
                ...
                // 🚀 MAGIA PURA: Aplastamos la ventana 3x3 en un solo pipeline ininterrumpido.
