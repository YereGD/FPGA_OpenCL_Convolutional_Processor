...
// 🚀 ¡NUEVO ANCHO DE BANDA DEL HOST! Ahora enviamos bloques de 16
...
// ========================================================================
// 🚀 LA MAGIA: ZERO-POINT FOLDING (Pre-cálculo Matemático en la CPU)
// ========================================================================
void fold_zero_points(std::vector<unsigned char>& w_data, std::vector<int>& b_data) {
    std::vector<signed char> new_w_signed(w_data.size()); 

    for (size_t i = 0; i < active_schedule.size(); i++) {
        ConvLayerDesc& layer = active_schedule[i];
        int out_c = layer.out_c;
        int in_c = layer.in_c;
        int w_z = layer.w_zero;
        int x_z = layer.x_zero;

        for (int oc = 0; oc < out_c; oc++) {
            int bias_idx = (layer.b_offset / 4) + oc;
            int current_bias = b_data[bias_idx];
            int zero_point_sum = 0;

            for (int k = 0; k < 9; k++) {
                for (int ic = 0; ic < in_c; ic++) {
                    int w_idx = layer.w_offset + (oc * 9 * in_c) + (k * in_c) + ic;
                    int original_w = w_data[w_idx];
                    
                    // 1. Nuevo Peso (w')
                    int w_prime = original_w - w_z;
                    new_w_signed[w_idx] = (signed char)w_prime;

                    // 2. Acumulamos el impacto para el Bias
                    zero_point_sum += (x_z * w_prime);
                }
            }
            // 3. Nuevo Bias (b')
            b_data[bias_idx] = current_bias - zero_point_sum;
        }
    }
    
    std::memcpy(w_data.data(), new_w_signed.data(), w_data.size());
    std::cout << "[HOST] Zero-Point Folding Completado. ALUTs liberados para SIMD x16." << std::endl;
}

void init_opencl_and_engine(unsigned char* w_data, int* b_data, size_t w_size, size_t b_size) {
    cl_int err; clGetPlatformIDs(1, &platform, NULL); clGetDeviceIDs(platform, CL_DEVICE_TYPE_ALL, 1, &device, NULL);
...
int main() {
    std::cout << "INICIANDO MOTOR FPGA V5 (SIMD x16 + ZERO FOLDING)..." << std::endl;
    ...
    // 🚀 APLICAMOS EL PLEGADO DE CEROS ANTES DE INICIAR LA FPGA
    fold_zero_points(rw, (std::vector<int>&)rb);
    
    init_opencl_and_engine(rw.data(), (int*)rb.data(), rw.size(), rb.size());

    cv::VideoCapture cap(0);
    if (!cap.isOpened()) { std::cerr << " Error webcam." << std::endl; return -1; }
    cap.set(cv::CAP_PROP_FRAME_WIDTH, 320); cap.set(cv::CAP_PROP_FRAME_HEIGHT, 240);

    unsigned char x_z = active_schedule[0].x_zero;
    int first_in_c = active_schedule[0].in_c;
    int input_bytes = 320 * 240 * 16; 
    cl_int err; cv::Mat frame, resized_img, rgb_img;

    while (true) {
        cap >> frame; if (frame.empty()) continue;
        cv::resize(frame, resized_img, cv::Size(320, 240)); cv::cvtColor(resized_img, rgb_img, cv::COLOR_BGR2RGB);

        unsigned char* mapped_in = (unsigned char*)clEnqueueMapBuffer(queue_read, d_buf_0, CL_TRUE, CL_MAP_WRITE, 0, input_bytes, 0, NULL, NULL, &err);
        std::fill(mapped_in, mapped_in + input_bytes, x_z);

        for (int y = 0; y < 240; ++y) {
            for (int x = 0; x < 320; ++x) {
                cv::Vec3b p = rgb_img.at<cv::Vec3b>(y, x);
                int base = (y * 320 + x) * first_in_c;
                mapped_in[base + 0] = p[0]; mapped_in[base + 1] = p[1]; mapped_in[base + 2] = p[2]; 
                if (first_in_c > 3) mapped_in[base + 3] = x_z;
            }
        }
        clEnqueueUnmapMemObject(queue_read, d_buf_0, mapped_in, 0, NULL, NULL); clFinish(queue_read);
        ...
        std::cout << "⏱️ Tiempo FPGA (SIMD x16): " << tiempo_ms << " ms (" << (1000.0/tiempo_ms) << " FPS)" << std::endl;

        unsigned char* mapped_out = (unsigned char*)clEnqueueMapBuffer(queue_write, d_buf_out, CL_TRUE, CL_MAP_READ, 0, CPU_OUT_SIZE, 0, NULL, NULL, &err);
        post_process_and_draw(frame, mapped_out);
        clEnqueueUnmapMemObject(queue_write, d_buf_out, mapped_out, 0, NULL, NULL); clFinish(queue_write);
